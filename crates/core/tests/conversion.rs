...
/// An attribute the renderer reads but the tag's mask omits silently reads
/// `None`, so adding one without widening the mask fails here.
#[test]
...
fn blockquote_content_start_survives_a_trailing_space_trim() {
  // The trim used to cut behind the blockquote's `content_start`, splicing the
  // quote prefix mid-token (`<d> etails>` instead of `> <details>`).
  let md = html_to_markdown(
...
fn pre_fence_opener_survives_a_trailing_space_trim() {
  // Same reach-back against a code fence: the trim ate the opener's trailing
  // newline, leaving `content_start` past the buffer end, so finalizing panicked.
  ...
  // The block-marker escapes that set this up (`#`, `-`, `>`) all reached it.
  for html in [
    "- x<pre><td></pre>",
    "> q<pre><td></pre>",
    "#  hash<pre><span></pre>",
...
fn spacing_check_tolerates_an_empty_text_node() {
  // `should_add_spacing_before_text` indexed byte 0 unconditionally. A `<td>`
  // inside `<pre>` opens an implied table, and closing the row emits an empty
  // text node while the last written byte is still content.
  let md = html_to_markdown("<pre><td>\n<th>6</th>\n<l>", HTMLToMarkdownOptions::default());
  assert_eq!(md, "```\n | 6\n```", "got: {md:?}");

  // Same shape without the `<pre>`, and via the streaming path.
  for html in [
    "<td>\n<th>6</th>\n<l>",
    "<pre><td>\n<th>x</th>\n<span>",
    "<pre><td></td><th></th>\n<l>",
  ] {
    let _ = html_to_markdown(html, HTMLToMarkdownOptions::default());
  }
}
