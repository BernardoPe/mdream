...
/// `(`, `)`, plus every byte of [`DESTINATION_ESCAPES`]. Keep it a superset.
...
impl ConvertState {
  ...
  /// Trim trailing spaces, never cutting behind [`Self::trim_floor`].
  #[inline]
  ...
  pub(crate) fn should_add_spacing_before_text(&self, last_byte: u8, text: &str) -> bool {
    ...
    // an empty text node behind, which used to panic here.
