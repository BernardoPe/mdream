...
/// Close targets, same table form so the scan loop makes no indirect call.
...
/// Open table-internal nodes a new `<tr>` may close.
const ROW_CLOSEABLE: [bool; 256] = tag_set(&[TAG_TD, TAG_TH, TAG_TR]);

/// Open table-internal nodes a new `<thead>`/`<tbody>`/`<tfoot>` may close.
...
/// Match `prop` at `index`, returning the offset of the value that follows.
/// One optional space, so `display:none` and `display: none` both match.
#[inline]
fn style_value_at(bytes: &[u8], index: usize, prop: &[u8]) -> Option<usize> {
  ...
  Some(end + usize::from(bytes.get(end) == Some(&b' ')))
}

/// Whether an inline `style` declares one of the hiding property/value pairs.
/// One pass with a first-byte dispatch rather than eight `str::contains` scans.
...
impl ConvertState {
  ...
  pub(crate) fn process_opening_tag(
    ...
    if self.has_plugins {
      ...
      if self.has_filter {
        // Hidden and exclude-matched elements drop their whole subtree and
        // inherit identically, so one marker covers both: a descendant costs an
        // Option check instead of rescanning every ancestor against every
        // selector.
