...
fn scan_tag<const EXTRACT: bool>(
  ...
  while i < chunk_length {
    ...
    if c == SLASH_CHAR && i + 1 < chunk_length && bytes[i + 1] == GT_CHAR {
      let (attributes, cell_span) = scan.finish(html_chunk, i);
      return (true, i + 2, attributes, true, cell_span);
    }
    if c == GT_CHAR {
      let (attributes, cell_span) = scan.finish(html_chunk, i);
      return (true, i + 1, attributes, self_closing, cell_span);
    }
...
fn push_attr(result: &mut Attributes, mask: u16, raw: &str, value: Option<&str>, span: &mut u8) {
  ...
  // `colspan` is only ever read as a small integer, so it lands in a scalar and
  // skips the name/value strings a map entry would allocate for every cell.
  // Plugins see all attributes, so the map still gets it under `ATTR_ALL`.
  if bit == ATTR_COLSPAN {
    *span = value
...
mod tests {
  ...
  #[test]
  fn colspan_reaches_the_scalar_and_stays_visible_to_plugins() {
    // Filtered masks read it as a number and skip the map entry's two strings.
    let (_, _, attrs, _, span) = process_tag_attributes("td colspan=\"3\">", 2, None, ATTR_COLSPAN);
    assert_eq!(span, 3);
    assert!(attrs.get("colspan").is_none());

    // Plugins force ATTR_ALL, where the attribute must still be readable.
    let (_, _, attrs, _, span) = process_tag_attributes("td colspan=\"3\">", 2, None, ATTR_ALL);
    assert_eq!(span, 3);
    assert_eq!(attrs.get("colspan").map(String::as_str), Some("3"));

    // A cell without one reports 0, which `cell_span` clamps to a single column.
    let (_, _, _, _, span) = process_tag_attributes("td>", 2, None, ATTR_COLSPAN);
    assert_eq!(span, 0);
  }
