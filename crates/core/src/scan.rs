...
/// `attr_mask` selects. Returns `(complete, new_position, attributes,
/// self_closing)`.
...
/// Store one attribute if `mask` selects its name. Rejection happens before any
/// lowercasing or entity decoding, so unwanted attributes cost no allocations.
#[inline]
fn push_attr(result: &mut Attributes, mask: u16, raw: &str, value: Option<&str>) {
  if !attr_wanted(mask, raw.as_bytes()) {
    return;
  }
  // Single-pass lowercase: the result is owned either way, so the uppercase
  // pre-scan would only add a redundant pass.
...
pub(crate) fn parse_attributes(attr_str: &str, mask: u16) -> Attributes {
  ...
  while i < len {
    ...
    match state {
      ...
      QUOTED_VALUE => {
        // Run to the closing quote instead of re-entering the six-way dispatch.
        while i < len && bytes[i] != quote_char {
          i += 1;
        }
        if i == len {
          // Unterminated quote: the attribute is dropped, as before.
          break;
        }
        push_attr(
          &mut result,
