...
/// Keep every attribute (any plugin that can read arbitrary names is active).
pub const ATTR_ALL: u16 = u16::MAX;

/// Map a raw (possibly mixed-case) attribute name to its `ATTR_*` bit.
/// Length-first so the common miss costs one compare.
#[inline]
