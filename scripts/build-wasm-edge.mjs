 ...
 *
 * Pipeline: wasm-pack (release, opt-level=s) -> wasm-opt -O3 --converge
 ...
 *
 * Converging is what pays on the wasm-opt side, not the level. Benchmark this
 * flag set on arm64, which the perf gate uses: it is worth 4.3% there and only
 * ~1.4% on x86_64, so a local x86 run reads as noise.
 */
...
console.log(`${wasmFile}: ${rawSize} -> ${optSize} bytes (wasm-opt -O3 --converge)`)
