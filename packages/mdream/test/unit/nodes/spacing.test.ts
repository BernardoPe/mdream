...
describe.each(engines)('spacing $name', (engineConfig) => {
  it('preserves new lines edge case', async () => {
  ...
  it('spacing between block elements inside a', async () => {
    const engine = await resolveEngine(engineConfig.engine)
    ...
    const markdown = htmlToMarkdown(html, { engine })
    expect(markdown).toBe('[Introduction Nuxt\'s goal is to make web development intuitive and performant with a great Developer Experience in mind.](/docs/getting-started/introduction)')
  })
  it('misc pre code', async () => {
    const engine = await resolveEngine(engineConfig.engine)
  ...
  it('comments between', async () => {
    ...
    const markdown = htmlToMarkdown(html, { engine })
    expect(markdown).toBe('Last updated on March 12, 2025')
  })

  it('adjacent links should have space between them', async () => {
    const engine = await resolveEngine(engineConfig.engine)
  ...
  it('block elements inside inline elements should have proper spacing', async () => {
    const engine = await resolveEngine(engineConfig.engine)
